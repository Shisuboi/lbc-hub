...
def build_triage_prompt(ads: list[dict], grounding: dict) -> str:
    ...
    return (
        ...
        "Distingue bien DEUX cas OPPOSÉS d'un prix bas :\n"
        "- PRIX DÉRISOIRE = PIÈGE : un prix qui n'est qu'une FRACTION INFIME d'un prix fonctionnel "
        "plausible (ex. un PC à 8 €, un iPhone à 20 € — une poignée d'euros pour un objet qui en vaut "
        "des centaines une fois sain) → objet cassé/PIÈCES/arnaque, marge ILLUSOIRE. "
        "→ 'passable', score bas, dig_deeper=false.\n"
        "- PRIX BAS MAIS CRÉDIBLE = AFFAIRE À CREUSER, PAS UNE ARNAQUE : un prix nettement SOUS LE "
        "MARCHÉ mais qui reste une fraction RÉALISTE de la valeur d'occasion (typiquement 40-70 % du "
        "neuf) sur un modèle RÉCENT et IDENTIFIABLE (ex. MacBook Air M3 15\" 2024 à 800 €, iPhone "
        "récent à moitié prix) est EXACTEMENT le profil d'une bonne affaire. NE LE PLOMBE PAS : "
        "c'est 'interesting', score ÉLEVÉ (75+) et **dig_deeper=true**. La sensation « TROP BEAU pour "
        "être vrai » ne baisse JAMAIS le score au triage — le risque d'arnaque se tranche à l'étape "
        "PHOTO (état réel + vendeur), pas ici. Ton rôle est de LAISSER PASSER l'affaire pour qu'elle "
        "soit vérifiée, pas de la tuer par méfiance.\n"
        ...
        "ÉCHELLE DE SCORE : 85-100 = clairement sous le marché, objet plausiblement sain, titre "
        "détaillé ; 60-84 = prix attractif ou au marché avec bonnes caractéristiques, à creuser ; "
        "40-59 = prix au marché sans avantage notable (mais si infos manquantes → dig_deeper=true) ; "
        "0-39 = EXPLICITEMENT mauvais : prix GONFLÉ au-dessus du marché, prix DÉRISOIRE (fraction "
        "infime = pièces/cassé), indices pièces/panne, ou titre VIDE. Un prix bas mais CRÉDIBLE "
        "n'est JAMAIS 0-39 : il monte en 75+ avec dig_deeper=true.\n"
