...
def build_triage_prompt(ads: list[dict], grounding: dict) -> str:
    ...
    return (
        ...
        "Sois donc exigeant :\n"
        "- Un prix anormalement bas par rapport au marché (ex. moins de 30 % du prix marché) n'est "
        "PAS une bonne affaire : c'est quasi toujours un objet cassé/défectueux, une arnaque-appât "
        "ou une erreur. Classe-la 'passable', score bas, dig_deeper=false.\n"
        "- Un titre trop vague ou trop court pour juger (modèle/état/capacité absents) ne permet "
        "pas d'évaluer sérieusement : écarte-la facilement → 'passable', score bas.\n"
        "- Ne réserve 'interesting' et un bon score QU'AUX annonces dont le prix est crédible ET "
        "le titre assez détaillé pour inspirer confiance. En cas de doute sur le sérieux d'une "
        "annonce, préfère 'passable'.\n"
